...
import com.example.project.palindrome.repository.PalindrometestRepository;

@Service
public class PalindrometestServiceImpl implements PalindrometestService {

	@Autowired
	PalindrometestRepository palindrometestRepository;
	...
	public String checkAndInsertPalindrome(String inputStr) {
		...
		Palindrometest strPalindrome = new Palindrometest();

		List<Palindrometest> strPalingList;
		// Below condition is for the first time
		if (palindrometestRepository.count() == 0) {
			strPalindrome.setPalindromeString(longestPolindromeString);
			palindrometestRepository.save(strPalindrome);
			return longestPolindromeString;
		} else {
			strPalingList = palindrometestRepository.findAll();
			strPalindrome = strPalingList.get(0);
			if (strPalindrome.getPalindromeString().length() < longestPolindromeString.length()) {
				strPalindrome.setPalindromeString(longestPolindromeString);
				palindrometestRepository.deleteAll();
				palindrometestRepository.save(strPalindrome);
			}
			return longestPolindromeString;
		}
